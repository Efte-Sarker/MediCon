...
2. Home Tabs
- Patient Dashboard: Serves as the central control screen displaying an emergency button, next appointment/consultation time, recent consulted doctor, and a reminder with the next medicine name and time.
- Doctors board: Category by department & symptom, consultation history, and a list of online doctors.
...
- AI Chat: Consultation chat using AI-Powered assistance, voice to transcription from consultation.
...
5. Symptom Triage
- Natural Language Symptom Ingestion: Accepts unstructured typed or spoken conversational input describing physical ailments, duration, and perceived pain levels.
...
11. Doctor Credentialing (Admin)
- Doctor Verification Queue: Reviews credentials submitted at Doctor signup (license number, specialty, supporting documents) before a Doctor account is granted full Doctor permissions.
- Backend-Enforced Approval Gate: Newly registered Doctor accounts remain in a pending state — blocked from all doctor-only actions — until approved.
- Internal-Only Surface: Administered entirely through a separately authenticated internal endpoint. There is no Admin login, tab, or screen within the Patient/Doctor mobile app.
...
# Admin (Internal Only)
- Reviews and approves/rejects Doctor credential submissions
- Accessed exclusively through a separately authenticated internal endpoint — never via the Patient/Doctor mobile app login
- Has no patient-facing or doctor-facing UI; not selectable at signup
